...
class ReviewService(BaseService):
    ...
    async def add_review(
        self, user_id: UserIdDep, product_id: int, data: ReviewsAddRequest
    ):
    ...
    async def add_review_images(
        self, user_id: UserIdDep, review_id: int, images: list[UploadFile]
    ):
        review = await self.get_review_with_check(review_id)
        if review.user_id != user_id:
            permissions = await AuthService(self.db).get_user_permissions(user_id)
    ...
    async def update_review(
        ...
        if review.user_id == user_id:
            ...
            await self.db.reviews.exit(
                data, exclude_unset=exclude_unset, id=review_id, user_id=user_id
            )
            await self.db.commit()
            return
